#!/usr/bin/env bash
#
# mayhem/test.sh — RUN astpretty's own test suite (deps already installed by mayhem/build.sh)
# and emit a CTRF (ctrf.io) summary. exit 0 iff failed==0. PATCH-grade oracle: the suite asserts
# exact pformat/pprint output strings (golden values), so a no-op patch that neuters the module
...
# the astpretty_run_tests ELF wrapper. We only RUN the suite. The suite is routed through that
...
# Put the in-image site dir (atheris + pytest) and the astpretty source tree on PYTHONPATH.
...
RUNNER="$SRC/astpretty_run_tests"
if [ ! -x "$RUNNER" ]; then
...
# Run the ENTIRE upstream suite (tests/astpretty_test.py — the whole tests/ tree). -p no:cacheprovider
# keeps the read-only image happy; -o addopts= drops upstream coverage addopts (covdefaults isn't a
# runtime dep of the suite's assertions).
LOG="$(mktemp)"
