#!/usr/bin/env bash
#
# mayhem/build.sh — build the astpretty Atheris fuzz harness + its standalone reproducer,
...
#      resolves entirely from it (pip --no-index --find-links). astpretty itself has no runtime
#      deps and is exercised as its editable source tree (repo root on PYTHONPATH — astpretty.py
#      is a single top-level module).
#   2. Compile launcher.c -> the ELF Mayhem target `astpretty_fuzzer` (Atheris is a Python
#      script; Mayhem needs an ELF cmd, and the gate needs DWARF < 4 — hence a compiled wrapper).
#   3. Build the same launcher as the standalone (run-once) reproducer `astpretty_fuzzer-standalone`.
#   4. Compile the pytest ELF runner wrapper `astpretty_run_tests` (so the sabotage oracle bites).
#
...
# wrapper, not the fuzzed Python; Atheris instruments the astpretty module itself at import time).
set -euo pipefail
...
#    manylinux wheel for this CPython. pytest runs astpretty's own pytest suite.
PKGS=(atheris pytest)
need_download=0
...
#    site dir holds atheris+pytest we SKIP the reinstall. astpretty itself stays the editable source
#    tree (repo root on PYTHONPATH) so a PATCH agent's edits to astpretty.py take effect with no
#    reinstall.
...
# astpretty is a single top-level module (astpretty.py) at the repo root, so the repo root itself
# goes on PYTHONPATH.
PYRUN="$SITE:$SRC"
...
PYTHONPATH="$PYRUN" "$PY" -c 'import atheris, astpretty, pytest; print("imports OK: astpretty")'
...
#    at run time (the Dockerfile sets ENV PYTHONPATH), so the Python side finds atheris + astpretty.
HARNESS="$SRC/mayhem/fuzz_ast.py"
echo ">> compiling astpretty_fuzzer (+ standalone) with DEBUG_FLAGS=$DEBUG_FLAGS"
$CC $DEBUG_FLAGS -DPYTHON="\"$PY\"" -DHARNESS="\"$HARNESS\"" \
    "$SRC/mayhem/launcher.c" -o "$SRC/astpretty_fuzzer"
...
$CC $DEBUG_FLAGS -DPYTHON="\"$PY\"" -DHARNESS="\"$HARNESS\"" \
    "$SRC/mayhem/launcher.c" -o "$SRC/astpretty_fuzzer-standalone"
...
$CC $DEBUG_FLAGS -DPYTHON="\"$PY\"" "$SRC/mayhem/run_tests.c" -o "$SRC/astpretty_run_tests"

echo ">> build.sh complete"
ls -la "$SRC/astpretty_fuzzer" "$SRC/astpretty_fuzzer-standalone" "$SRC/astpretty_run_tests"
